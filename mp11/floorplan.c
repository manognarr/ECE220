...
//Introduction: In this program, we developed functions that would allow building an efficient floorplan. 
//We use trees and we are able to access members in the structs provided to change the modules, height, 
//width, cutline, etc. of the tree and also rotate and recut the tree.
...
int is_in_subtree(node_t* a, node_t* b) {
  //check if node 'a' is NULL
  if(a == NULL)
   return 0;
   //check if the subtrees root ('b') is the node ('a'), and if it is, return 1
 if(a == b )
   return 1;
   //recursively check if 'b' is within 'a' subtree
...
void rotate(node_t* ptr) {
 ...
 if(ptr->module != NULL && is_leaf_node(ptr)) //check if module is not null and if it is a leaf node
 {
   temp = ptr->module->w; //swap the width and the height of the node's module
   ptr->module->w = ptr->module->h;
   ptr->module->h = temp;
   return;
 }

 //recursively rotate the children of the node until leaf is reached
 rotate(ptr->right); 
 rotate(ptr->left);
}
...
void recut(node_t* ptr) {
 ...
 //check if cutline of given node is 'V' and if it is, change it to 'H'
 if(ptr->cutline == V)
   ptr->cutline = H;
   //check if cutline of given node is 'H' and if its, change it to 'V'
 else
   ptr->cutline = V;
 return;
}
...
void swap_module(node_t* a, node_t* b) {
 ...
 module_t* temp = a->module; //declare temp pointer and set it to a module
 //swap a and b module
 a->module = b->module;
 b->module = temp;
}
...
void swap_topology(node_t* a, node_t* b) {
 ...
 //declare parenttemp pointer and initialize to 'a' parent
 node_t *parenttemp = a->parent; 
 
 //swap a and b parent
a->parent = b->parent;
b->parent = parenttemp;
 
 //check if a and b are in the correct swapped locations
 if(a->parent->left == b)
 {
   a->parent->left = a;
 }
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  if(ptr == NULL) return; //if ptr is NULL, return

 //recursively call function to traverse the tree
 postfix_traversal(ptr->left, nth, expression); 
 postfix_traversal(ptr->right, nth, expression);
 if (ptr->cutline!=UNDEFINED_CUTLINE){ //check if cutline is not undefined
   expression[*nth].cutline=ptr->cutline; //if so, set the expression cutline to current node's cutline
   expression[*nth].module=NULL; //set expression's module to NULL.
 }
 
 else
 {
   expression[*nth].module=ptr->module; //if not, set expression's module to current node's module
 }
 *nth = *nth +1; //increment nth to traverse tree
}
...
{
 if(ptr->right == NULL && ptr->left == NULL) //check if both children are NULL
   return ptr->module->resource; //return each node's resource
   return get_total_resource(ptr->left) + get_total_resource(ptr->right); //otherwise, return the added resources of the children
}
...
node_t* init_slicing_tree(node_t* par, int n) {
 ...
 ptr <- malloc a new node of node_t
// Base case
if(n == num_modules - 1) {
… // assign the data to the module pointed by ptr
return ptr
}
// Internal node
… // assign the data to the module pointed by ptr
// Right child
ptr->right <- malloc a new node of node_t as the right child of ptr
… // assign the data to the module pointed by ptr->right
...
if (n==num_modules-1){    //if the node is a leaf, then it has undefinded cutline
 ptr->module=modules+n;  //the module value is then changed with a pointer
 ptr->parent=par; //set current node's parent to par
 ptr->cutline=UNDEFINED_CUTLINE; //set current node's cutline to UNDEFINED_CUTLINE
 return ptr; //return the pointer, ptr
}
 
ptr->module=NULL;         //if it is an internal node, then the module val is null
ptr->parent=par;          //the cutline is V since the tree is left skewed
ptr->cutline=V;
 
ptr->right=(node_t*)malloc(sizeof(node_t)); //create new node and set it current node's right
ptr->right->module=modules+n; //set current node's right module using the modules array which has the module
ptr->right->cutline=UNDEFINED_CUTLINE; //set current node's right cutline to UNDEFINED_CUTLINE
ptr->right->parent=ptr; //set current node's right parent to ptr
 
ptr->left=init_slicing_tree(ptr, n+1); //set current node's left by recursively calling function
return ptr; //return the pointer, ptr.
